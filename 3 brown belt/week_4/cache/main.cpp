...
void TestCaching2(const Library& lib) {
    ...
    for (int i = 0; i < lib.book_names.size(); ++i){
        ICache::BookPtr book_ptr = cache->GetBook(lib.book_names[i]);
        ASSERT_EQUAL(book_ptr->GetName(), lib.book_names[i]);
        ASSERT_EQUAL(book_ptr->GetContent(), "Dummy content of the book " + lib.book_names[i]);
    }
    ASSERT_EQUAL(unpacker->GetUnpackedBooksCount(), 10);
